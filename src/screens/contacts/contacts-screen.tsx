...
export function ContactsScreen() {
  ...
  return (
    ...
    >
            ...
            {filtered.map((c) => (
              <motion.div key={c.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); if (confirm(`Delete ${c.name}?`)) deleteMutation.mutate(c.id) }}
      ...
      <ContactDialog open={showCreate} initial={EMPTY_FORM} title="New Contact" isSubmitting={createMutation.isPending}
